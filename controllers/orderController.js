...
export const getSuperAdminOrders = catchAsyncErrors(
  async (req, res, next) => {
    try {
      const page = Number(req.query.page) || 1;
      const limit = Number(req.query.limit) || 10;
...
// Get Single Buyers orders
// export const buyerOrders = catchAsyncErrors(async (req, res, next) => {
...
// Get Buyer Orders with pagination
export const buyerOrders = catchAsyncErrors(async (req, res, next) => {
  const { buyer, seller, page = 1, limit = 10 } = req.body;
  ...
  // Seller filter inside items array
  if (seller) {
